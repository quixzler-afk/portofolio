...
const projects = [
    ...
    },
    {
        ...
        description: "Website pengaduan masyarakat Kecamatan Pamulang untuk berbagai kategori laporan publik.",
        images: [
            "image/lp-1.png",
            "image/lp-2.png",
            "image/lp-3.png",
...
function initializeProfilePhotoViewer() {
    ...
    if (profilePhoto && profileImg) {
        ...
        profilePhoto.addEventListener('click', () => {
            openProfilePhotoModal(profileImg.src, profileImg.alt);
        });
    }
}

// Open profile photo in modal
function openProfilePhotoModal(imageSrc, imageAlt) {
    ...
    // Set modal content
    modalTitle.textContent = 'Profile Photo';
    ...
    // Hide slider buttons for single image
...
function initializeModal() {
    ...
    closeBtn.onclick = () => {
        ...
        // Show slider buttons again
        document.querySelectorAll('.slider-btn').forEach(btn => btn.style.display = 'flex');
    };
    ...
    document.addEventListener('keydown', (e) => {
        if (modal.style.display === 'flex') {
            if (e.key === 'ArrowLeft' && currentProject) changeImage(-1);
            if (e.key === 'ArrowRight' && currentProject) changeImage(1);
            if (e.key === 'Escape') {
                modal.style.display = 'none';
                document.body.style.overflow = 'auto';
                // Show slider buttons again
...
function openModal(project) {
    ...
    // Show slider buttons for projects
...
// Scroll animations - Updated for light mode
window.addEventListener('scroll', () => {
    const navbar = document.querySelector('.navbar');
    if (window.scrollY > 50) {
        navbar.style.background = 'rgba(255, 255, 255, 0.95)';
        navbar.style.boxShadow = '0 4px 25px rgba(14, 165, 233, 0.12)';
    } else {
